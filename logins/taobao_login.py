...
class TB_Spider:
    ...
    def login(self):
        ...
        try:
            button = self.wait.until(EC.presence_of_element_located((By.CLASS_NAME, 'h')))
            button.click()
            sleep(2)
            username_sender = self.wait.until(EC.presence_of_element_located((By.ID, 'fm-login-id')))
            username_sender.send_keys(self.user)
            sleep(1)
            password_sender = self.wait.until(EC.presence_of_element_located((By.ID, 'fm-login-password')))
