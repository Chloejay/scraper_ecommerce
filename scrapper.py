...
class Parse_TB:
    def __init__(self,
        ...
        self._from_data = from_data  # Fix IP blocked
    ...
    def _get_cookie(self) -> Optional[Text]:
        ...
        if not redirect:
            pprint(
                "Succeed getting cookies, to save to {}".format(
                    self.cookies_text))
            self._save_cookies()
        else:
            raise RuntimeError(
                "User login FAILED! Error:{}".format(
                    response.text))
    ...
    def get_static_content(self,
        ...
        for i in range(len(file)):
            ...
            try:
                title = soup.find(
                    "h3", class_="tb-main-title").get_text().strip()
                line_through_price = soup.find(
                    "em", class_="tb-rmb-num").get_text()
                商品标题.append(title)
    ...
    def get_static_content_update(
        ...
        try:
            for i in range(len(url)):
                ...
                title = browser.find_element_by_class_name(
                    'tb-main-title').text
                line_through_price = browser.find_element_by_id(
                    'J_StrPrice').find_element_by_class_name('tb-rmb-num').text
                comment_count = browser.find_element_by_id(
                    "J_RateCounter").text
        ...
        return pd.DataFrame(
            {"商品链接": url, "商品标题": 商品标题, "划线价": 划线价, "累计评论": 累计评论})
...
if __name__ == "__main__":
    ...
    with Pool(processes=4) as pool:
        taobao_data = pool.map(tb_parser.get_static_content, [
            tb_excel.loc[:, "商品链接"][:].values],)
